...
Tuli internal server error ja en päässyt eteenpäin joten googlasin. Lähteestä 7 Selvisi, että jostain syystä tässä tehtävässä '--' kommentointimerkki ei toimi ja se pitää korvata '#' merkillä(voiko olla, että tässä jotenkin kommentointi in estetty '--' merkillä?). Lisäksi toinen arvo pitää muuttaa 'NULL' arvoksi. (testasin kirjoittaa alla olevaa komentoa suoraan selaimeen eikä se toiminut, en tiedä miksi. Muuttamalla komento zapissa GET pyyntöön se toimi)
